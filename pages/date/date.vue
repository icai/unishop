...
<view class="box" v-if="cycledate">
  <view class="box-flex" style="background: #f7f7f7;border-radius: 10rpx;">
    ...
    <view class="flex-item" style="text-align: right;">
      <view class="item-content" @tap="doDay" data-key="right">
  ...
  <view class="box-flex">
    <view class="flex-item" v-for="(item, key) in currentDayList" :key="key">
      <view :data-day="item.id" :data-week="item.week" @tap="item.no_optional ? '': 'selectDay'" :class="'item-content   ' + (item.checked ? 'checked' : '') + ' ' + (item.no_optional  ? 'no-optional' : '')" style>{{item.id == 0 ? '' : item.id}}</view>
    </view>
  </view>
</view>
</template>
