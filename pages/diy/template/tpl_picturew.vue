...
<view v-else :class="'fui-picturew row-' + diyitem.params.row" :style="'background:' + diyitem.style.background">
    <block v-if="diyitem.params.showtype==1">
       <swiper :interval="interval" :duration="duration" :style="'background:' + diyitem.style.background + ';padding:20rpx 0;'">
        <block>
          <swiper-item class="fui-picturew" v-for="(childitem, index) in diyitem.data_temp" :key="index">
          <view class="item" v-for="(pic_item, index2) in childitem" :key="index2">
            <view class="navigator" @tap="navigate" :data-url="(pic_item.linkurl=='/pages/commission/index'?'../../commission/pages/index':pic_item.linkurl)" :data-phone="pic_item.phone" :data-appid="pic_item.appid" :data-appurl="pic_item.appurl" :style="'padding:' + (diyitem.style.paddingtop==0?0:diyitem.style.paddingtop + 'rpx') + ' ' + (diyitem.style.paddingleft==0?0:diyitem.style.paddingleft + 'rpx')">
    ...
    <block v-else>
      <view :style="'padding:' + (diyitem.style.paddingtop==0?0:diyitem.style.paddingtop + 'rpx') + ' ' + (diyitem.style.paddingleft==0?0:diyitem.style.paddingleft + 'rpx') + ';box-sizing:border-box;'" class="item" v-for="(childitem, index) in diyitem.data" :key="index">
        <view class="navigator" @tap="navigate" :data-url="(childitem.linkurl=='/pages/commission/index'?'../../commission/pages/index':childitem.linkurl)" :data-phone="childitem.phone" :data-appid="childitem.appid" :style="'padding:' + (diyitem.style.paddingtop==0?0:diyitem.style.paddingtop + 'rpx') + ' ' + (diyitem.style.paddingleft==0?0:diyitem.style.paddingleft + 'rpx')">
