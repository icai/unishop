...
<view class="page navbar" v-if="show">
        ...
        <block v-if="refund.rtype==1 || refund.rtype==2">
            <view class="fui-cell-group">
                <view class="fui-cell">
                    ...
                    <navigator :class="'fui-cell-remark ' + ( refund.status!=3?'noremark':'' )" :url="'/pages/order/express/number?id=' + order.id + '&refundid=' + refund.id">{{ refund.statusstr }}</navigator>
                </view>
    ...
    <view class="fui-title" v-if="order.refundstate==1">
        协商详情
    </view>
    ...
    <view class="fui-cell-group">
        <view class="fui-cell" hover-class="none">
            ...
            <view :class="'fui-cell-remark noremark ' + ( order.refundstate==1||order.refundstate==2?'':'hidden' )">{{ rtypeArr[rtypeIndex] }}</view>
        ...
        <view class="fui-cell">
            ...
            <view class="fui-cell-info"><input type="text" placeholder="选填" :class="'fui-input ' + ( order.refundstate==0?'':'hidden' )" :value="content" @change="change" data-name="content"></input></view>
        ...
        <view class="fui-cell" v-if="rtypeIndex!=2">
         ...
         <view :class="'fui-cell-remark noremark ' + ( order.refundstate==1?'':'hidden' )">￥{{ price }}</view>
        </view>
        ...
        <view class="fui-cell" v-if="order.refundstate==1||order.refundstate==2">
   ...
   <view class="fui-footer text-right">
       ...
       <block v-if="order.refundstate==2">
           <view v-if="refund.status==3&&refund.status==5" class="btn btn-danger">确认收到换货物品</view>
           <navigator v-if="refund.status==3&&refund.status==5" url><view class="btn btn-primary" open-type="navigate">查看换货物流</view></navigator>
           <navigator style="width:auto" v-if="refund.status==3||refund.status==4" class="btn btn-primary btn-sm" :url="'/pages/order/express/number?id=' + order.id + '&refundid=' + refund.id">{{ refund.express?'修改':'填写' }}快递单号</navigator>

           <view v-if="refund.status==0" class="btn btn-danger-o btn-sm" @tap="edit">修改申请</view>

           <view v-if="refund.status!=5" class="btn btn-default-o btn-sm" @tap="refundcancel">取消申请</view>
