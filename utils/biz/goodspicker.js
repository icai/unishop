...
module.exports = {
  ...
  buyNow: function (event, $this, page) {
    ...
    var giftid = $this.data.giftid; //判断周期购
    ...
    core.get('goods/get_picker', {
      ...
      if (result.goods && result.goods.giftinfo) {
        if (result.goods.giftinfo.length == 1) {
          $this.setData({
            giftid: result.goods.giftinfo[0].id
          });
        }
      }
      ...
      if (result.diyform) {
        $this.setData({
          diyform: {
            fields: result.diyform.fields,
            f_data: result.diyform.lastdata
          }
        });
      }
  ...
  specsTap: function (event, $this) {
    ...
    specs.forEach(function (e) {
      title += e.title + ';'; // optionTitle += e.id + '_';

      optionids.push(e.id);
    }); // var newOptionids = optionids.sort();

    var newOptionids = optionids.sort(this.sortNumber);
    optionTitle = newOptionids.join('_'); // optionTitle = optionTitle.substring(0, optionTitle.length - 1);
