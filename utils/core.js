...
module.exports = {
  ...
  json: function (routes, args, callback, hasloading, ispost, session) {
    ...
    if (callback) {
      op.success = function (res) {
        ...
        if (res.errMsg == 'request:ok') if (typeof callback === 'function') {
  ...
  upload: function (uploadCallback) {
    ...
    wx.chooseImage({
      success: function (res) {
        self.loading('正在上传...');
        var url = self.getUrl('util/uploader/upload', {
          file: 'file'
        });
        var tempFilePaths = res.tempFilePaths; //  if( typeof(chooseCallback)==='function'){
  ...
  onShareAppMessage: function (url, customtitle) {
    var app = getApp(),
        sysset = app.getCache('sysset'),
        share = sysset.share || {},
        userinfo_id = app.getCache('userinfo_id'),
        mid = '';
