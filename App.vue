...
export default {
  ...
  removeCache: function (key) {
    ...
    try {
      wx.removeStorageSync(key + this.$options.globalData.appid);
    } catch (e) {
      rt = false;
    }
  ...
  setCache: function (key, value, expire) {
    ...
    try {
      wx.setStorageSync(key + this.$options.globalData.appid, data);
    } catch (e) {
      rt = false;
    }
  ...
  getCache: function (key, defaultValue) {
    ...
    try {
      data = wx.getStorageSync(key + this.$options.globalData.appid);
  ...
  getConfig: function () {
    if (this.$options.globalData.api !== null) {
      return {
        api: this.$options.globalData.api,
        approot: this.$options.globalData.approot,
        appid: this.$options.globalData.appid
      };
    }

    var ext = wx.getExtConfigSync();
    this.$options.globalData.api = ext.config.api;
    this.$options.globalData.approot = ext.config.approot;
    this.$options.globalData.appid = ext.config.appid;
    return ext.config;
  },
